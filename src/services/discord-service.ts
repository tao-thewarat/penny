...
import type { ImageRepository } from '../repositories/image-repository.ts'
...
export function createDiscordService(
  ...
  async function handleMessage(msg: Message): Promise<void> {
    ...
    try {
      ...
      await reply(
        msg,
        'ขออภัยครับ ตอนนี้ทำรายการไม่สำเร็จ ลองใหม่อีกครั้งนะครับ',
      )
    }
  }
  ...
  async function route(
    ...
    switch (interpretation.intent) {
      case 'log_entry': {
        try {
          const slipKeys = await Promise.all(
            images.map((image) =>
              imageRepo.upload({
                body: image.data,
